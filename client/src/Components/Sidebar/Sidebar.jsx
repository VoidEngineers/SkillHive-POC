...
const Sidebar = () => {
  ...
  useEffect(() => {
    ...
    else if (path === "/learning-progress") setActiveTab("Learning Progress");
    else if (path === "/about") setActiveTab("About Us");
    else if (path.includes(`/${user.reqUser?.username}`)) setActiveTab("Profile");
        ...
        <div className="flex flex-col justify-between h-full py-6 px-6">
            ...
            <nav className="space-y-2">
              {mainu.map((item) => (
                ...
                >
                  ...
                  {!isSidebarCollapsed && (
                    <span className={`ml-3 text-sm ${activeTab === item.title ? "font-semibold" : "font-medium"}`}>
                      {item.title}
                    </span>
                  )}
                  
                  {activeTab === item.title && !isSidebarCollapsed && (
                    <motion.div
          ...
          <div className="mt-4 relative">
            ...
            >
              ...
              {!isSidebarCollapsed && (
                <span className="ml-3 text-sm font-medium">More</span>
              )}
            </motion.div>
          ...
          {!isSidebarCollapsed && user?.reqUser && (
            <div className="mt-6 flex items-center p-3.5 rounded-xl cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-800/60">
              <img 
                src={user.reqUser.image || "https://via.placeholder.com/40"} 
                alt={user.reqUser.username}
                className="w-9 h-9 rounded-full object-cover border-2 border-gray-200 dark:border-gray-700"
              />
              <div className="ml-3 truncate">
                <p className="text-sm font-semibold text-gray-900 dark:text-gray-200">{user.reqUser.username}</p>
              </div>
            </div>
          )}
        </div>
